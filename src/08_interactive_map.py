...
layers.worst5 = L.geoJSON(kelurahanData, {{
  ...
  onEachFeature: function(f, layer) {{
    ...
    const label = L.divIcon({{
      className: '',
      html: `<div style="color:#ff8a80;font-family:Inter,sans-serif;font-size:11px;font-weight:600;text-shadow:0 1px 4px rgba(0,0,0,0.8);white-space:nowrap;">${{f.properties.NAMOBJ}}</div>`,
      iconSize: [0, 0],
      iconAnchor: [0, 0]
    }});
